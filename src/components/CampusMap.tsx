...
export default function CampusMap({ onSelectBlock }: { onSelectBlock: (block: string) => void }) {
  ...
  return (
    <div className={`fixed inset-0 h-[100dvh] w-screen z-0 overflow-hidden ${isDarkMode ? 'bg-zinc-900' : 'bg-[#f4f4f5]'}`}>
      
      {/* ---HIDDEN FLOATING ROUTING PANEL --- */}
      <div className={`hidden absolute top-6 left-6 z-[1000] w-72 backdrop-blur-md p-4 rounded-xl shadow-xl border transition-colors ${isDarkMode ? 'bg-zinc-900/90 border-zinc-700' : 'bg-white/90 border-zinc-200'}`}>
